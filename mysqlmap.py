...
def sqlmap_g_nohuman(http_url_or_file):
	...
	if re.match("(http://)|(https://)",http_url_or_file):
		...
		sqlmap_string='''/usr/share/sqlmap/sqlmap.py --tor --tor-type=socks5 --check-tor -g "site:%s inurl:php|asp" --smart --batch -v 3 --threads 4 --random-agent''' % domain_url
...
def sqlmap_craw(origin_http_url_or_file):
	...
	if re.match("(http://)|(https://)",origin_http_url_or_file):
		...
		sqlmap_string='''/usr/share/sqlmap/sqlmap.py --tor --tor-type=socks5 --check-tor -u "%s" --crawl=3 --smart -v 3 --threads 4 --batch --random-agent''' % origin_http_url
		print("sqlmap_string is:%s" % sqlmap_string)
	else:
...
def sqlmap_g_human(http_url_or_file):
	...
	if re.match("(http://)|(https://)",http_url_or_file):
		...
		sqlmap_string='''/usr/share/sqlmap/sqlmap.py --tor --tor-type=socks5 --check-tor -m GoogleScraper_origin_http_domain_url_list.txt -v 3 --smart --batch --threads 4 --random-agent'''
		print("sqlmap_string is:%s" % sqlmap_string)
		os.system("/usr/bin/python2.7 %s" % sqlmap_string)
	else:
